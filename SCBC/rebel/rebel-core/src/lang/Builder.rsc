...
tuple[set[Message], set[Built]] loadAll(loc modLoc, 
  ...
  tuple[set[Message], BuiltInternal, set[Module]] build(Module modul) {
    println("building <modul.modDef>");
    ImporterResult importResult = loadImports(modul, cachedParse);
    
    //println("imports <importResult>");
  ...
  //, bprintln("b.inlinedMod <b.inlinedMod>"), 
  //bprintln("b.normalizedMod <b.normalizedMod>"), 
  //bprintln("b.refs <b.refs>"), 
  //bprintln("b.resolvedTypes <b.resolvedTypes>"), 
  //bprintln("loadUsedBy(b.inlinedMod) <loadUsedBy(b.inlinedMod)>")
  }>;
}
