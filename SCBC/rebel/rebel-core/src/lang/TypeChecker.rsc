...
TypeCheckerResult checkTypes(Module modul, set[Module] imports) {
  ...
  if (modul has spec) {
    ...
    for (EventDef ev <- modul.spec.events.events) {
      ...
      configParamsInEvent = ("<p.name>" : p.tipe | /Parameter p := ev.configParams);
      //println("configParamsInEvent: <configParamsInEvent>");
      Context ctx = context(nested("<ev.name>", paramsInEvent + configParamsInEvent, rootScope));
