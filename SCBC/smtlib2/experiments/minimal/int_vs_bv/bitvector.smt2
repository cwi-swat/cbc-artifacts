...
(declare-fun counter (State) (_ BitVec 6))
(declare-fun param (State) (_ BitVec 6))

(define-fun increment ((current State) (next State)) Bool
	(and 
		(= (param next) (_ bv1 6))
		(= (counter next) (bvadd (counter current) (param next)))
	) 
)
...
(define-fun decrement ((current State) (next State)) Bool
	(and 
		(= (param next) (_ bv1 6))
		(= (counter next) (bvsub (counter current) (param next)))
	)
)
...
(define-fun transitionCondition ((current State) (next State)) Bool
	(and 
		...
		(= next (bvadd current (_ bv1 5)))
	)
)

(define-fun safetyCondition ((state State)) Bool
	(and 
		(bvslt (counter state) (_ bv25 6))
		(bvsgt (counter state) (_ bv0 6))
	)
)

(define-fun initialCondition ((state State)) Bool
	(and 
		(= (counter state) (_ bv10 6))
		(= state (_ bv0 5))
	)
)
