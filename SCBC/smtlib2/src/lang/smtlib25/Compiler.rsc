...
str compile(defineSort(str name, list[str] sorts, Sort sort)) = "(define-sort <name> (<("" | "<it> <s>" | s <- sorts)>) <compile(sort)>)"; // when bprintln("Compiling " + "<sort>");
...
str compile(defineFunction(name, params, returnType, body)) = "(define-fun <name> (<compile(params)>) <compile(returnType)>\n  <compile(body)>)";
...
str compile(dataTypeDef(str typeName, list[Constructor] cons)) = "<typeName> <intercalate(" ", ["(<compile(c)>)" | Constructor c <- cons])>";

str compile(combinedCons(str name, list[SortedVar] sortedVars)) = "<name> <intercalate(" ", ["<compile(sv)>" | SortedVar sv <- sortedVars])>"; 
...
// Sorts
str compile(SortedVar var) = "(<var.name> <compile(var.sort)>)";
str compile(list[SortedVar] params) = ("" | "<it> (<param.name> <compile(param.sort)>)" | SortedVar param <- params); 
...
str compile(custom(name)) = name;
str compile(combined(list[Sort] sorts)) = "(<compile(sorts)>)"; // when bprintln("COMBINED" + sorts);
...
str compile(and([])) = "true";
str compile(and(list[Formula] clauses)) = "(and <intercalate("\n    ", [compile(c) | c <- clauses])>)";
...
str compile(ite(condition, whenTrue, whenFalse)) = "(ite <compile(condition)>\n    <compile(whenTrue)> <compile(whenFalse)>)";   

// From ints
str compile(neg(val)) = "(- <compile(val)>)";
str compile(sub(lhs, rhs)) = "(- <compile(lhs)> <compile(rhs)>)";
//str.++ 
str compile(add(lhs, rhs)) = "(str.++  <compile(lhs)> <compile(rhs)>)" when "<lhs>" == "h_AMOUNT_STRING";
...
// From Record
str compile(is(str cons)) = "is-<cons>";
// ((_ update-field fieldName) param_this newFieldValue))
str compile(updateField(fieldName, recordValue, newValue)) = "((_ update-field <compile(fieldName)>) <compile(recordValue)> <compile(newValue)>)";

// From array
str compile(select(arr, idx)) = "(select <compile(arr)> <compile(idx)>)"; // when bprintln(idx);
...
default str compile(node n) {
  throw "Unable to compile `<n>`"; 
}
