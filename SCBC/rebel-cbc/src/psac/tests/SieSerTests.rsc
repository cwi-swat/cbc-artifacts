...
test bool findDirectAcceptActions() = {
  SieResult result = runPsacAssertAndIterateModels(accountLoc, smtAssert = sieAcceptSer);
  ...
  //[<"Close","Close">,<"Deposit","Deposit">,<"Withdraw","Deposit">]
  
};

//test bool findFailFastActions() = {
//  SieResult result = runPsacAssertAndIterateModels(accountLoc, smtAssert = sieRejectSer);
//  
...
bool equal(list[&A] found, list[&A] expected) = {
  if(found == expected) {
    ...
    throw "Missing: <expected - found>. Extra <found - expected>. <found> is not equal to <expected>";
  };
};