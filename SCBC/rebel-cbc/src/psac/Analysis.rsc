...
list[Command] rebelSmtPrelude = declareRebelTypes() 
  ...
  + defineDateOperator("\>") + defineDateOperator("\<") + defineDateOperator("\>=") + defineDateOperator("\<=")
  ;
    //var(simple("i")));
...
str translateToSmt(Built mainBuilt, set[Built] builts) {
  ...
  // try to load in Z3
  println("Try to load SMT code in Z3");
  runInZ3(smtCode);
...
public str psacAssert = "(declare-const e1 Event)
...
public str psacAssertAccept = "(declare-const e1 Event)
         '(declare-const e2 Event)
         '(declare-const state_post_e1 State)
         '(declare-const state_before State)
...
public str psacAssertDecline = "(declare-const e1 Event)
         '(declare-const e2 Event)
         '(declare-const state_post_e1 State)
         '(declare-const state_before State)
         '(declare-const some_state State)
...
//@memo
SieResult runPsacAssertAndIterateModels(loc specificationLoc, str smtAssert = psacAssert) {
  ...
  try {
    ...
    while(sat) {
      ...
      //println([ "\t<m> <model[m]>"  | model := getModel(z3, ["pre_e1_before", "pre_e2_before", "pre_e2_post_e1"]), m <- model ]);
      ...
      //str (int) scp = str(int i) {return "<i>";};
      
      println([ "\t<m> <model[m]>"  | model := getModel(z3, ["(pre e2 state_before)", "(pre e2 state_post_e1)"]), m <- model ]);
      
      //println("Solving time: <getSolvingTime(z3)>");  //  seems  broken
      
      //m = [ visit(model[m]) {
      //  case c :foundValues => c
      //} | m <- model];
      //println(m);
...
// Used for exposed tesgts
SieResult sieAnalysis(loc stateMachineSmtLocation, set[str] eventNames  str smtAssert = psacAssert) {
  str smt = readFile(stateMachineSmtLocation);
  int z3 = startSolver();
  try {
    println(z3);
    ...
    runSolver(z3,  smt);  
    //println("fed file to z3");
    runSolver(z3,  smtAssert); 
    //println("fed assert to z3");
    //runSolver(z3, debugAsserts);
    //println("fed debug asserts to z3");
    ...
    while(sat) {
      ...
      // somehow I get parse errors when querying exposed... Workaround
      smtResult = runSolverAndExpectResult(z3, "(get-value (e2_exposed))");
      println(smtResult);
    ...
    allEvents = { capitalize("<event>") | event <- eventNames };
    combinations = { <e1,e2> | e1 <- allEvents, e2 <- allEvents};
    plainIncompatibleEvents = { <e1,e2> | <e1,e2,_> <- incompatibleEvents };
    set[tuple[str,str]] compatibleEvents = combinations - plainIncompatibleEvents;
    SpecInfo specInfo = <stateMachineSmtLocation, allEvents, {}, size(allEvents), 0>;
    return <sort(plainIncompatibleEvents), sort(compatibleEvents), sort(allEvents), specInfo>;
    } catch ex: throw (ex);
  finally {
    stopSolver(z3);
  ...
  //return <[],[],[], <stateMachineSmtLocation, {}, {}, 0, 0> >;
}
