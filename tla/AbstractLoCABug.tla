...
define
  ...
  Operations == {CC!Deposit(1), CC!Withdraw(1), CC!Interest(2), CC!Query}
...
fair process tr \in resources
  variables 
            voted = {}, \* 2PC READY transactions
            rcommitted = {},\* 2PC COMMIT
            aborted = {},  \* 2PC ABORT
            queued = {}, \* 2PC COMMIT, not yet applied
            inProgress = <<>>, \* [tId |-> .., op |-> .., delayed |-> BOOLEAN]
            delayed = <<>>, \* 2PC INIT (msg stashed)
            state = InitialState[self] \* initial state for object
begin
TR_INIT:
