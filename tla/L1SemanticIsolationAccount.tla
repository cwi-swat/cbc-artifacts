--------------------------- MODULE L1SemanticIsolationAccount  ---------------------------
...
\* Experiment eval (indirection)
PreWithdraw(amount, balance) == (balance - amount) >= 0
EffWithdraw(amount, balance) == balance - amount

PreDeposit(amount, balance) == OK
EffDeposit(amount, balance) == balance + amount

PreInterest(amount, balance) == OK
EffInterest(amount, balance) == balance * amount

PreQuery(balance) == OK \* balance = amount
EffQuery(balance) == balance
RetValQuery(balance) == balance
